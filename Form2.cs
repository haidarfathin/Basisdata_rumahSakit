...
namespace hospital_1
{
    public partial class Form2 : Form
    {
        ...
        {
            this.search_data_pasien(textBox5.Text);
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            if (textBox3.Text == "")
            {
                ...
                dataCommand = new MySqlCommand("INSERT INTO patients (fullname, nik, birth_date, birth_place, sex, occupation) VALUE(@fullname, @nik, @birth_date, @birth_place, @sex, @occupation)", this.con);
            }
            else
            {
                dataCommand = new MySqlCommand("UPDATE patients SET fullname = @fullname, nik = @nik, birth_date = @birth_date, birth_place = @birth_place, sex = @sex, occupation = @occupation WHERE id = @patient_id", this.con);
            ...
            this.con.Close();
            this.clear_form();
            this.load_data_pasien();
        }

        private void load_single_pasien(int patient_id)
        {
            this.con.Open();
            MySqlCommand dataCommand = new MySqlCommand("SELECT * FROM patients WHERE id = @patient_id", this.con);
            ...
            if (dataReader.HasRows)
            {
                ...
                textBox6.Text =
                dataReader.GetString(dataReader.GetOrdinal("occupation"));
            }
            else
            {
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            this.load_single_pasien(Int16.Parse(patient_id));
        }

        private void drop_single_pasien(int patient_id)
        {
            this.con.Open();
            MySqlCommand dataCommand = new MySqlCommand("DELETE FROM patients WHERE id = @patient_id", this.con);
            try
            {
                dataCommand.Parameters.AddWithValue("@patient_id", patient_id);
        ...
        private void button4_Click_1(object sender, EventArgs e)
        {
            ...
            if (answer == DialogResult.Yes)
            {
                var rowIndex = dataGridView1.CurrentCell.RowIndex;
                var patient_id = dataGridView1.Rows[rowIndex].Cells[0].Value.ToString();
                this.drop_single_pasien(Int16.Parse(patient_id));
                this.load_data_pasien();
            }
